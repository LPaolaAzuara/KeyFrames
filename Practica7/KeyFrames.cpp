...
void cargaAnimación(const char* filename = "Animacion.txt") {
...
// Función para imprimirr el archivo txt
void imprimeAnimación(const char* filename = "Animacion.txt") {
	std::ifstream file(filename);
	if (!file.is_open()) {
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Animacion maquina de estados | Paola Azuara", nullptr, nullptr);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	if (key == GLFW_KEY_Q && action == GLFW_PRESS) {
		resetElements();    // Resetea elementos
		cargaAnimación();    // Carga de animacion
		imprimeAnimación();   // Imprime el contenito del archivo txt
	}
